class Teller {
	...
	public void openAccount() {
		...
		int foundNameIndex = bankManageHandler.findNameGetIndex(tempName); // 입력받은 이름의 중복 위치 인덱스를 넘겨받고 
		...
		bankManageHandler.accountToClient(foundNameIndex, account); // 계좌 정보를 고객에 연결 
	}
	
	public void closeAccount() {
		
		String typedName = bankManageIOHandler.typeName(); // 이름입력 받고
		int foundNameIndex = bankManageHandler.findNameGetIndex(typedName); // 이름찾고 ( 찾으면 인덱스값 or 없으면 -1)
		...
		bankManageHandler.deleteSelectedAccount(foundNameIndex, leftBalanceResult); // 잔액 확인 후 계좌 삭제 
	}
